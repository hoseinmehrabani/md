...
if most_moving_sperm:
    ...
    total_distance = most_moving_sperm['total_distance']
    
    # فرض بر این است که VCL، VAP و VSL مطابق با پارامترهای موجود محاسبه شود.
    vsl = total_distance / len(most_moving_sperm['positions']) if most_moving_sperm['positions'] else 0
    vcl = total_distance  # برای محاسبه VCL باید مسیر واقعی محاسبه شود
    vap = total_distance / len(most_moving_sperm['positions']) if most_moving_sperm['positions'] else 0

    # محاسبات LIN و STR
    lin = (vsl / vcl) * 100 if vcl != 0 else None
    str_value = (vsl / vap) * 100 if vap != 0 else None

    result_message = (
        f"لوکیشن اسپرمی که بیشترین جابجایی داشته و زنده است: {final_position} با مجموع جابجایی {total_distance}\n"
        f"درصد خطی بودن حرکت اسپرم: {lin:.2f}%" if lin is not None else "محاسبه درصد خطی نامعتبر است.\n"
        f"درصد مستقیم‌الخط بودن حرکت اسپرم: {str_value:.2f}%" if str_value is not None else "محاسبه درصد مستقیم‌الخط بودن نامعتبر است."
    )
else:
    result_message = "هیچ اسپرم زنده‌ای با جابجایی بالا پیدا نشد."

# نمایش نتیجه در پیام‌باکس
Tk().withdraw()
messagebox.showinfo("نتیجه تحلیل", result_message)
