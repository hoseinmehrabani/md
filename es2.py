...
# پارامترهای تشخیص حرکت و ردیابی
...
# ماسک برای ردیابی مسیر و اطلاعات اسپرم‌ها
mask = np.zeros_like(old_frame)
tracked_sperms = {i: {'positions': deque(maxlen=10), 'speeds': deque(maxlen=10), 'total_distance': 0, 'alive': False} for i in range(len(p0))}
...
while cap.isOpened():
    ...
    if p1 is not None and p0 is not None:
        ...
        for i, (new, old) in enumerate(zip(good_new, good_old)):
            ...
            # شناسایی اسپرم زنده با حداقل سرعت
            speed = distance
            if speed > 2:  # شرط اسپرم زنده
                if not tracked_sperms[i]['alive']:
                    alive_sperm_count += 1
            ...
            # رسم دایره‌ی ردیابی با خط نازک‌تر
            mask = cv2.circle(mask, (int(a), int(b)), 3, (0, 255, 0), 1)
            frame = cv2.add(frame, mask)

        # شماره‌گذاری اسپرم‌های زنده
        for idx, data in tracked_sperms.items():
            if data['alive'] and data['positions']:
                pos = data['positions'][-1]
                cv2.putText(frame, f"{idx}", (int(pos[0]), int(pos[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
...
Tk().withdraw()
messagebox.showinfo("تعداد اسپرم‌های سالم", f"تعداد اسپرم‌های سالم و مولد: {alive_sperm_count}")
